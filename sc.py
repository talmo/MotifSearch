"""
sc.py -- Self-Consistency of a Motif (MotifSearch)

This program will calculate the self-consistency of a motif as measured by the total probability
that a specified number of transcription factor molecules will bind to the motif given a set of
starting sites. It will then iterate by incorporating those probabilities into a position-specific
count matrix in order to observe how a given motif is consistent unto itself.

For more information see the wiki: http://erilllab.biosci.umbc.edu/wiki/index.php5/MotifSearch

Usage:
    sc.py GENOME_FILE MOTIF_SEQUENCES -i ITERATIONS -score SCORING_METHOD -cn COPY_NUMBER -debug -pickle
    * GENOME_FILE: a FASTA formatted (.fna) genome sequence file, without the extension
    * MOTIF_SEQUENCES: a plain-text (.txt) file with one sequence per line; sequences must be aligned/same-length
    * ITERATIONS: how many times to loop; defaults to 1 iteration
    * SCORING_METHOD: bvh = Berg and von Hippel (default); is = information scoring; pssm = position-specific scoring matrix
    * COPY_NUMBER: number of TF molecules; defaults to the number of sequences in the original motif
    * debug flag: if present will output status of simulation to console
    * pickle flag: if present will output a "pickled" array of the simulation's data to a file at the end of all iterations

Example:
    sc.py NC_000913 crp -i 100 -score bvh -cn 208 -debug -pickle
        => Genome: ../TFBSDB/genomes/NC_000913.fna
        => Motif: ./motifs/crp.txt
        => Pickle: ./data/NC_000913_crp_bvh_CN208_I100.pkl

Notes:
    * The data is pickled at the end of the simulation for separate analysis.
    * This program uses pure Python, no external libraries.
    => This is done intentionally to maintain compatibility with PyPy which, for the type of 
    calculations done here, provides 2-4x performance increase versus CPython.
    * See analyze.py to see how the data is interpreted.
    * Currently, only Berg and von Hippel scoring correctly scales to binding energies.
    * The output data structure that is pickled is created in the __main__ routine (bottom of this file).
"""
...
def iterate(genome, pscm, scoring_matrix, copy_number):
    ...
    for pos, sequence in window_generator:
        # Calculate probability given the energy
...
def get_scores_and_energies(sequences, scoring_matrix, beta, temperature, width):
    ...
    for sequence in sequences:
        ...
        scores.append(score)
        energy = TRAP(score, width)  # TODO: Make energy scaling method parametrized
...
def save_pickle(data_to_pickle):
    ...
    debug("Pickled the data to: data/%s" % data_to_pickle["filename"])
...
if __name__ == '__main__':
    ...
    scoring_method = args.score if args.score is not None else "bvh"
    ...
    for pscm, Z in zip(PSCMs, Zs):
        ...
        for sequence in (site.sequence for site in original_motif):
            ...
            energy = TRAP(score, original_motif.width)  # TODO: make energy scaling method parametrized
